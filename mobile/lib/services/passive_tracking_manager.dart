...
class PassiveTrackingManager {
  static final PassiveTrackingManager _instance = PassiveTrackingManager._internal();
  ...
  List<TrackingPoint> _pointBuffer = [];
  Timer? _stillTimer;
  ...
  void start() {
    ...
    debugPrint("PassiveTrackingManager started.");
  }
  ...
  void _onNewTrackingPoint(TrackingPoint point) {
    if (_currentState == TrackingState.monitoring || _currentState == TrackingState.inProgress) {
      _pointBuffer.add(point);
    }
    // Periodically send data if a trip is in progress
    if (_currentState == TrackingState.inProgress && _pointBuffer.length >= 15) {
  ...
  void _onActivityChanged(PredictedActivity activity) {
    debugPrint("Activity Changed: ${activity.type.name}, Confidence: ${activity.confidence}, State: $_currentState");

    // --- Trip Start Logic ---
    // If we're idle and become still, we start monitoring for a potential trip.
    if (_currentState == TrackingState.idle && activity.type == ActivityType.still) {
    ...
    else if (_currentState == TrackingState.monitoring && activity.type != ActivityType.still) {
    ...
    if (_currentState == TrackingState.inProgress && activity.type == ActivityType.still) {
      _stillTimer?.cancel();
      _stillTimer = Timer(const Duration(minutes: 3), _handleTripEnd);
    } else if (_currentState == TrackingState.inProgress && activity.type != ActivityType.still) {
  ...
  Future<void> _handleTripEnd() async {
    ...
    debugPrint("User has been still for 3 minutes. Ending trip $_currentTripId");
    try {
  ...
  Future<void> userConfirmedTripStart(String temporaryTripId) async {
    ...
    try {
      final permanentTripId = await _trackingService.confirmTripStart(temporaryTripId);
      _currentTripId = permanentTripId; // Update to the permanent ID from the server
