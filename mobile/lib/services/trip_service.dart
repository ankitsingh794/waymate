...
class TripService {
  ...
  Future<PaginatedTrips> getAllTrips({int page = 1, int limit = 10, String? status, String? destination}) async {
    try {
      final queryParams = {
        'page': page.toString(),
  ...
  Future<Trip> updateTripDetails(String tripId, Map<String, dynamic> details) async {
    try {
      final response = await _apiClient.patch('trips/$tripId/details', body: details);
      return Trip.fromJson(response['data']['trip']);
    } catch (e) {
  ...
  Future<Trip> updateTripStatus(String tripId, String status) async {
    try {
      final response = await _apiClient.patch('trips/$tripId/status', body: {'status': status});
      return Trip.fromJson(response['data']['trip']);
    } catch (e) {
  ...
  Future<String> acceptTripInvite(String token, {String? ageGroup, String? gender, String? relation}) async {
    try {
        final body = {
            'token': token,
  ...
  Future<void> updateMemberRole(String tripId, String memberId, String role) async {
      try {
          await _apiClient.patch('trips/$tripId/members/$memberId/role', body: {'role': role});
      } catch (e) {
  ...
  Future<void> updateMyMemberDetails(String tripId, Map<String, String> details) async {
      try {
  ...
  Future<void> updateDayItinerary(String tripId, int day, List<String> activities) async {
      try {
          await _apiClient.patch('trips/$tripId/itinerary/$day', body: {'activities': activities});
      } catch (e) {
  ...
  Future<Map<String, String>> syncOfflineTrips(List<Map<String, dynamic>> trips) async {
    try {
      final response = await _apiClient.post('trips/sync', body: {'trips': trips});
